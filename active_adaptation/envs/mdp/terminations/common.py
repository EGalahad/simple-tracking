...
class crash(Termination):
    def __init__(
        ...
        **kwargs,
        ...
        self.count = torch.zeros(
            self.num_envs, len(self.body_indices), device=self.env.device
        )
    ...
    def update(self):
        in_contact = (
            self.contact_sensor.data.net_forces_w[:, self.body_indices].norm(dim=-1)
            > 1.0
        )
...
class cum_error(Termination):
    def __init__(self, env, thres: float = 0.85, min_steps: int = 50):
        ...
        self.error_exceeded_count = torch.zeros(
            self.env.num_envs, 1, device=self.env.device, dtype=torch.int32
        )
...
class impact_exceeds(Termination):
    ...
    def __call__(self) -> torch.Tensor:
        impact_force = self.contact_sensor.data.net_forces_w_history[
            :, :, self.body_ids
        ]
...
class impedance_pos_error(Termination):
    ...
    def __call__(self):
        error = (self.asset.data.root_link_pos_w - self.command_manger.des_pos_w)[
            :, :2
