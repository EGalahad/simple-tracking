...
class angvel_xy_l2(Reward):
    def __init__(
        self, env, weight: float, enabled: bool = True, body_names: str = None
    ):
        super().__init__(env, weight, enabled)
        self.asset: Articulation = self.env.scene["robot"]
...
class body_upright(Reward):
    ...
    def __init__(self, env, body_name: str, weight, enabled=True):
        ...
        self.down = torch.tensor([[0.0, 0.0, -1.0]], device=self.device).expand(
            self.num_envs, len(self.body_id), 3
        )

    def compute(self) -> torch.Tensor:
        g = quat_apply_inverse(
            self.asset.data.body_link_quat_w[:, self.body_id], self.down
        )
...
class undesired_contact_force_xy(Reward):
    ...
    def compute(self):
        ...
        contact_forces = (contact_forces[:, :, :2].norm(dim=-1) - self.thres).clamp_min(
            0.0
        )
