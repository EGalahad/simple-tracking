...
def run_training_stage(
    cfg: DictConfig, return_queue: multiprocessing.Queue = None
) -> str:
...
def main(cfg: DictConfig):
    ...
    for i, stage_name in enumerate(cfg.stages):
        ...
        if process.exitcode != 0:
            print(
                f"ERROR: Child process for stage '{stage_name}' exited with code {process.exitcode}."
            )
        ...
        try:
            ...
            print(
                f"ERROR: Could not retrieve run_path from child process for stage '{stage_name}'. Error: {e}"
            )
