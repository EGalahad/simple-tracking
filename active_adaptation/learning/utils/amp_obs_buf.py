...
def _get_history_obs(obs: torch.Tensor, history_steps: list[int]) -> torch.Tensor:
    ...
    obs_indices = current_indices.unsqueeze(1) - history_offsets.unsqueeze(
        0
    )  # (num_frames, num_history_steps)

    return padded_obs[obs_indices].reshape(
        num_frames, -1
    )  # Flatten to (num_frames, num_history_steps * obs_dim)
...
class joint_pos_history_amp(AMPObservation):
    ...
    def compute(self):
        ...
        return _get_history_obs(
            joint_pos, self.history_steps
        )  # (num_frames, num_history_steps * num_joints)
...
class body_pos_b_history(AMPObservation):
    ...
    def compute(self):
        root_link_pos_w = self.buffer.body_link_pos_w[
            :, self.root_body_idx
        ]  # (num_frames, 3)
        root_link_quat_w = self.buffer.body_link_quat_w[
            :, self.root_body_idx
        ]  # (num_frames, 4)
        ...
        # Get body positions
        body_link_pos_w = self.buffer.body_link_pos_w[
            :, self.body_indices
        ...
        return _get_history_obs(
            body_pos_b, self.history_steps
        )  # (num_frames, num_history_steps * num_bodies * 3)
...
class body_lin_vel_b_history(AMPObservation):
    ...
    def compute(self):
        ...
        body_link_lin_vel_w = self.buffer.body_link_lin_vel_w[
            :, self.body_indices
        ]  # (num_frames, num_bodies, 3)

        # Transform to body frame
        body_lin_vel_b = quat_apply_inverse(
            root_link_quat_w_yaw.unsqueeze(1), body_link_lin_vel_w
        )
...
class body_ori_b_history(AMPObservation):
    ...
    def compute(self):
        ...
        body_link_quat_w = self.buffer.body_link_quat_w[
            :, self.body_indices
        ]  # (num_frames, num_bodies, 4)

        # Transform to body frame
        body_quat_b = quat_mul(
            quat_conjugate(root_link_quat_w_yaw)
            .unsqueeze(1)
            .expand_as(body_link_quat_w),
            body_link_quat_w,
        )
...
class body_ang_vel_b_history(AMPObservation):
    ...
    def compute(self):
        ...
        body_link_ang_vel_w = self.buffer.body_link_ang_vel_w[
            :, self.body_indices
        ]  # (num_frames, num_bodies, 3)

        # Transform to body frame
        body_ang_vel_b = quat_apply_inverse(
            root_link_quat_w_yaw.unsqueeze(1), body_link_ang_vel_w
        )

        return _get_history_obs(
            body_ang_vel_b, self.history_steps
        )  # (num_frames, num_history_steps * num_bodies * 3)
