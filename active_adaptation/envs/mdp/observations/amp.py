...
class _history(Observation):
    ...
    def __init__(self, env, history_steps: list[int] = [0]):
        ...
        self.obs_this_step = torch.zeros(
...
class joint_pos_history_amp(_history):
    def __init__(self, joint_names: str = ".*", **kwargs):
        self.env = kwargs["env"]
        ...
        self.joint_pos = torch.zeros(
            self.num_envs, 2, self.num_joints, device=self.device
        )
...
class body_lin_vel_b_history(_history):
    ...
    def update(self):
        ...
        body_lin_vel_b = quat_apply_inverse(
            root_link_quat_w.unsqueeze(1), body_link_lin_vel_w
        )
...
class body_ang_vel_b_history(_history):
    ...
    def update(self):
        ...
        body_ang_vel_b = quat_apply_inverse(
            root_link_quat_w.unsqueeze(1), body_link_ang_vel_w
        )
